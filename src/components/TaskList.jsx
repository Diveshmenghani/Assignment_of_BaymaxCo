...
const TaskList = () => {
  ...
  /**
   * Apply filters to tasks based on status and search term
   * 
   * @param {Array} taskList - List of tasks to filter
   * @param {string} status - Filter status ('all', 'complete', 'incomplete')
   * @param {string} term - Search term for filtering by title
   */
  const applyFilters = (taskList, status, term) => {
    let result = [...taskList];
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'tasks') {
        try {
          ...
          applyFilters(updatedTasks, filterStatus, searchTerm);
        } catch (err) {
  ...
  // applyFilters function moved to the top of the component

  /**
   * Handle status filter change
   * 
   * @param {string} status - New filter status
   */
  const handleFilterChange = (status) => {
    setFilterStatus(status);
    applyFilters(tasks, status, searchTerm);
  };
  ...
  const handleSearchChange = (e) => {
    ...
    setSearchTerm(term);
    applyFilters(tasks, filterStatus, term);
  };
    ...
    <div className="bg-white p-4 rounded-lg shadow max-h-96 overflow-y-auto">
      ...
      <div className="mb-4 space-y-2">
        <div className="flex items-center space-x-2">
          <FaFilter className="text-gray-500" aria-hidden="true" />
          <span className="text-sm font-medium text-gray-700">Filter:</span>
          <div className="flex space-x-1">
            <button 
              onClick={() => handleFilterChange('all')} 
              className={`px-2 py-1 text-xs rounded ${filterStatus === 'all' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            ...
            <button 
              ...
              className={`px-2 py-1 text-xs rounded ${filterStatus === 'complete' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
              aria-pressed={filterStatus === 'complete'}
            ...
            <button 
              ...
              className={`px-2 py-1 text-xs rounded ${filterStatus === 'incomplete' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 w-full p-2 text-sm border rounded focus:ring-2 focus:ring-blue-500 focus:outline-none"
