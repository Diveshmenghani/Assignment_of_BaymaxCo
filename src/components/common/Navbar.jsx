...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (userId && email) {
        const logData = {
          id: `logout-${Date.now()}`, // Unique ID for the log entry
          userId: userId,
          username: email,
          role: userRole || 'user',
          action: "logout",
        ...
        // Update existing logs with logout time
        const existingLogs = JSON.parse(localStorage.getItem('userLogs') || '[]');
        
        // Find the matching login entry (if any) to update with logout time
        const updatedLogs = existingLogs.map(log => {
          if (log.userId === userId && log.username === email && !log.logoutTime && log.action === "login") {
        ...
        console.log("User logout recorded:", logData);
      }
